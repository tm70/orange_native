...
const GameRequests: React.FC<Props> = ({ navigation, route }) => {
    ...
    const renderItem = ({ item }) => {
        ...
        if (item.status == 'InProgress') {
            ...
            onPress = () =>
                navigation.navigate(item.game_type, {
                    game_id: item.id,
            ...
            onPress = () =>
                Alert.alert('Cancel Request?', 'Would you like to cancel this game request?', [
                    {
                        text: 'Yes',
                        onPress: () => {
                            cancelGameRequest(userid, item.id, token);
            ...
            onPress = () =>
                Alert.alert('Respond to Request?', 'How would you like to respond to this game request?', [
                    {
                        text: 'Accept',
                        onPress: () => {
                            respondGameRequest(userid, item.id, 'Accept', token);
                    ...
                    },
                    {
                        text: 'Decline',
                        onPress: () => {
                            respondGameRequest(userid, item.id, 'Decline', token);
                            item.status = 'Cancelled';
                            rerender(!r);
                        },
                    },
        ...
        return <GameRequestButton item={item} bio={bios[item.opponent_id]} onPress={onPress} />;
    };
