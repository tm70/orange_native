...
export default ({ streams }) => {
    const RTCViewRendered = ({ userId, stream }) => {
        if (stream) {
            return <RTCView objectFit="cover" style={styles.blackView} key={userId} streamURL={stream.toURL()} />;
        }
    ...
    switch (streamsCount) {
        case 1:
            RTCListView = <RTCViewRendered userId={streams[0].userId} stream={streams[0].stream} />;
            break;
